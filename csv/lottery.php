...
$sql = 'SELECT c.CityName,m.ShopName,t.phone,p.name,t.createTime FROM lottery t '.
'left join market m on m.ShopID = t.marketId left join city c on c.CityID = t.cityId left join prize p on p.id = t.giftId order by t.cityId,t.marketId,t.giftId';
...
while ($row = mysql_fetch_assoc($res)) {
	...
	if ($isMac) {
        $str .= $cityname.','.$shopname.','.$phone.','.$name.','.$createTime . "\n";
    }else{
        $str .= $cityname.','.$shopname.','.$phone.','.$name.','.$createTime . "\n";
    }
}
